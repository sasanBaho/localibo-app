...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey || apiKey === "re_your_api_key_here") {
    ...
    return NextResponse.json({ error: "Email service is not configured. Please contact us at hi@localibo.com." }, { status: 503 });
  }
  ...
  try {
    const { error } = await resend.emails.send({
      from: "Localibo Feedback <noreply@localibo.com>",
